...
class Networking {
  ...
  Map mapResponseSeasonal = {};
  List lisResponseSeasonal = [];
  ...
  Future jikaApiSeasonalAnime() async {
    http.Response apiResponse;
    apiResponse = await http.get(Uri.parse('$jikanApiURL/seasons/now'));
    if (apiResponse.statusCode == 200) {
      mapResponseSeasonal = json.decode(apiResponse.body);
      lisResponseSeasonal = mapResponseSeasonal['data'];
    }
    return lisResponseSeasonal;
  }
}
